...
// $result = $sth->fetchAll();
    ...
    $stmt = $dbh->prepare("SELECT * FROM fa_ykjp_sclc as a,fa_ykjp_ddgl as b WHERE a.scddbh=b.ddbh AND a.status='开料'"); 
    $stmt->execute();
    $result = $stmt->fetchAll();


    $stmt1 = $dbh->prepare("SELECT * FROM fa_ykjp_sclc as a,fa_ykjp_kl as c WHERE a.scddbh=c.kl_ddbh ORDER BY kl_sysl"); 
    $stmt1->execute();
