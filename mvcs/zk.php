...
// $result = $sth->fetchAll();
    ...
    $stmt = $dbh->prepare("SELECT * FROM fa_ykjp_zk ORDER BY kl_zptime desc"); 
    $stmt->execute();
    $result = $stmt->fetchAll();


    $stmt1 = $dbh->prepare("SELECT * FROM fa_ykjp_ddgl as a,fa_ykjp_sclc as b WHERE a.ddbh=b.scddbh AND b.lcxs like '%c%'"); 
    $stmt1->execute();
...
//echo "<pre>";print_r($result1);echo "<pre>";
  ...
  $smarty->assign('title','钻孔');
  $smarty->display('tpl/zk.tpl');
?>